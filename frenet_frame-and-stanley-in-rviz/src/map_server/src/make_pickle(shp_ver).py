...
df=gpd.read_file("/home/nsclmds/catkin_ws/src/frenet_frame-and-stanley-in-rviz/src/map_server/src/map/playground(short).shp")

# 좌표 변환이 필요한 경우 주석 삭제
# df.to_crs(epsg=5179, inplace=true)
...
for i in range(len(df.x)):
  data_x=[]
  data_y=[]
  data_x.append(coor_dict[i]['x'])
  data_y.append(coor_dict[i]['y'])
...
with open('playground(short)_route.pkl', 'wb') as handle:
