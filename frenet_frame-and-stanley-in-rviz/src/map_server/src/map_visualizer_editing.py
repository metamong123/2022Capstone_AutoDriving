...
if __name__ == "__main__":
	...
	global_cv = Converter(global_file, 2000, r=255/255.0, g=236/255.0, b=139/255.0, a=0.8, scale=0.2)

	if not use_map.horizontal_parking_map_num==0:
		for i in range(use_map.horizontal_parking_map_num):
			globals()["parking_cv_{}".format(i)]=Converter(globals()["parking_file_{}".format(i)], 3000, r=228 / 255.0, g=233 / 255.0, b=237 / 255.0, a=0.8, scale=0.2)
	if not use_map.diagonal_parking_map_num==0:
		for i in range(use_map.diagonal_parking_map_num):
			globals()["parking_cv_{}".format(i)]=Converter(globals()["parking_file_{}".format(i)], 3000, r=228 / 255.0, g=233 / 255.0, b=237 / 255.0, a=0.8, scale=0.2)
	if not use_map.delivery_map_num==0:
		for i in range(use_map.delivery_map_num):
			globals()["delivery_cv_{}".format(i)]=Converter(globals()["delivery_file_{}".format(i)], 3000, r=228 / 255.0, g=233 / 255.0, b=237 / 255.0, a=0.8, scale=0.2)
