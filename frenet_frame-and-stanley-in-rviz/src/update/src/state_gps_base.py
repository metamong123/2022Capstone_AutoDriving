...
if __name__ == "__main__":
	...
	while not rospy.is_shutdown():
		...
		quat_gps = tf.transformations.quaternion_from_euler(0, 0, yaw_gps)
		tf_broadcaster.sendTransform((x_gps, y_gps, 1.5),quat_gps,rospy.Time.now(),"/car_" + str(id), "/map")
		...
		m.pose.position.x = x_gps
		m.pose.position.y = y_gps
		m.pose.position.z = 0.3
		m.pose.orientation = Quaternion(*quat_gps)
