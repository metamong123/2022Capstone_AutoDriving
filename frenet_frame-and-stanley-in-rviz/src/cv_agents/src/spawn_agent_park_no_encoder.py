...
# parking3
obs_init1 = Object(x=962575.68525061, y=1959246.37897911, yaw=2.84853436, L=2, W=2)
obs_init2 = Object(x=962577.02808145, y=1959251.08300435, yaw=2.84853436, L=2, W=2)
...
if __name__ == "__main__":
	...
	parking_pub=rospy.Publisher("/rviz/parking_paths", MarkerArray, queue_size=1)
	...
	# 	nodes['global']  = pickle.load(f)
	# with open("/home/nsclmds/catkin_ws/src/2022Capstone_AutoDriving/frenet_frame-and-stanley-in-rviz/src/map_server/src/route_parking1.pkl", "rb") as f:
	# 	nodes['parking']  = pickle.load(f)
	# nodes['parking'][1]={}
	# nodes['parking'][1]=nodes['parking'][0]
	# with open("/home/nsclmds/catkin_ws/src/2022Capstone_AutoDriving/frenet_frame-and-stanley-in-rviz/src/map_server/src/route_parking2.pkl", "rb") as f:
	# 	park_2= pickle.load(f)
	# 	nodes['parking'][2]=park_2[0]
	# nodes['parking'][3]={}
	# nodes['parking'][3]=nodes['parking'][2]
	...
	state = State(x=mapx['global'][ind], y=mapy['global'][ind], yaw=mapyaw['global'][ind], v=1, dt=0.1)
	...
	parking_path=Converter(r=0, g=100/255.0, b=100/255.0, a=0.4, scale= 0.5)
	while not rospy.is_shutdown():
		...
		if (mode == 'global') and ((my_wp[mode] >= 245) and (my_wp[mode] < 250)): ################parking mode 시작 웨이포인트 넣기
			...
			for park_i in range(0,5,2):
				...
				# if prev_park_ind ==0:
				# 	fp=ParkingPath()
				# 	fp.x=mapx['parking'][:link_len['parking'][park_i]]
				# 	fp.y=mapy['parking'][:link_len['parking'][park_i]]
				# 	fp.yaw=mapyaw['parking'][:link_len['parking'][park_i]]
				# else:
				# 	fp=ParkingPath()
				# 	fp.x=mapx['parking'][link_len['parking'][prev_park_ind]:link_len['parking'][park_i]]
				# 	fp.y=mapy['parking'][link_len['parking'][prev_park_ind]:link_len['parking'][park_i]]
				# 	fp.yaw=mapyaw['parking'][link_len['parking'][prev_park_ind]:link_len['parking'][park_i]]
				...
				if collision_check(fp,obs_info,0,0,0)==False:
					...
					print("cccccccccccchoose: "+str(park_i))
					mode='parking'
					rospy.set_param('car_mode', mode)
					break
		...
		# 		if collision_check(fp,obs_info,0,0,0)==False:
		# 			link_ind['parking']=park_i
		# 			print("cccccccccccccccccchoose: "+str(park_i))
		# 			mode='parking'
		# 			rospy.set_param('car_mode', mode)
		# 			break
		...
		parking_pub.publish(parking_path.ma)
		# control_pub.publish(msg)
