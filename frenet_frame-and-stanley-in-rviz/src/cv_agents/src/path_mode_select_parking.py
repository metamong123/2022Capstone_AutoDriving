	...
	while not rospy.is_shutdown():
		...
		if mode=='diagonal_parking':
			...
			for park_i in range(use_map.horizontal_parking_map_num):
				...
				fp = MakingPath()
				fp.x=use_map.horizontal_parking_path[park_i][0]
				fp.y=use_map.horizontal_parking_path[park_i][1]
				fp.yaw=use_map.horizontal_parking_path[park_i][2]
				print("위치" + str(park_i) + "번 차량존재유무 : " + str(collision_check(fp,obs_info,0,0,0)))
			...
			if collision_check(fp,obs_info,0,0,0)==True:
				for park_i in range(parking_ind,use_map.horizontal_parking_map_num,1):
					fp_1=MakingPath()
					fp_1.x=use_map.horizontal_parking_path[park_i][0]
					fp_1.y=use_map.horizontal_parking_path[park_i][1]
					fp_1.yaw=use_map.horizontal_parking_path[park_i][2]
					print("위치" + str(park_i) + "번 차량존재유무 : " + str(collision_check(fp_1,obs_info,0,0,0)))
