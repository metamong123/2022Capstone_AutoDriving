...
rn_id[5] = {'global': [0,1,2,3,4,5,6], 'parking':[0,1]}
...
def get_ros_msg(x, y, yaw, v, a, steer, id, t):
...
if __name__ == "__main__":
	...
	nodes['parking'][1]={'x':nodes['parking'][0]['x'][::-1],'y':nodes['parking'][0]['y'][::-1],'yaw':nodes['parking'][0]['yaw'][::-1],'s':nodes['parking'][0]['s'][::-1]}
	for i in range(10):
		for j in nodes['parking']:
			for k in nodes['parking'][j]:
				nodes['parking'][j][k]=np.append(nodes['parking'][j][k],np.array([nodes['parking'][j][k][-1]]))
	nodes['global'][6]={}
	...
	ind = 0
	movement='forward'
	target_speed = {'forward': 10.0/3.6, 'backward':-10.0/3.6}
	state=State(x=mapx[mode][ind], y=mapy[mode][ind], yaw=mapyaw['global'][ind], v=1, dt=0.1)
	# state.x=obj_msg.x
	# state.y=obj_msg.y
	...
	while not rospy.is_shutdown():
		...
		# YOUR CODE HERE
		if (mode=='parking') & (link_ind['parking']==1):
			if my_wp['parking']<=link_len['parking'][0]:
				movement='backward'
		else:
			movement='forward'

		path, opt_ind = frenet_optimal_planning(si, si_d, si_dd, sf_d, sf_dd, di, di_d, di_dd, df_d, df_dd, obs_info, mapx[mode], mapy[mode], maps[mode], opt_d, target_speed[movement])
		...
		if opt_ind == -1: ## No solution!
			...
			print("현재 링크 번호: "+ str(link_ind[mode])+", mode: " + str(mode) + ", target_speed: "+str(target_speed[movement]))
			...
			error_pa = target_speed[movement] - state.v
			error_da = state.v - prev_v
			error_ia += target_speed[movement] - state.v
			kp_a = 0.5
			kd_a = 0.7
			ki_a = 0.01
		...
		#v_list.append(v)
		print("현재 speed = " + str(state.v) + ",steer = " + str(steer) + ",a = "+str(a))
		# prev_v = state.v
		# state.x=obj_msg.x
		# state.y=obj_msg.y
		# state.yaw=obj_msg.yaw
		...
		df_d = 0
		df_dd = 0
		# if mode=='parking' & link_ind==1:
		# 	t=
		# else:
		# 	t=0
		# vehicle state --> topic msg
		msg = get_ros_msg(state.x, state.y, state.yaw, state.v, a, steer, id=id, t=0)
