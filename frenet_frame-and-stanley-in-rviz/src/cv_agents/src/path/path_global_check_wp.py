...
def sub_and_pub():
	# global opt_frenet_pub,cand_frenet_pub,waypoint_pub,dir_pub,global_path_pub,col_pub,col_msg,obstacle_sub,state_sub,accel_sub,mode_sub
	global s,d,x,y,road_yaw,dir,state,si,si_d,si_dd,sf_d,sf_dd,di,di_d,df_dd,di_dd,df_d,opt_d,prev_opt_d
...
if __name__ == "__main__":
	...
	opt_frenet_pub = rospy.Publisher("/rviz/optimal_frenet_path_1", MarkerArray, queue_size=1)
	cand_frenet_pub = rospy.Publisher("/rviz/candidate_frenet_paths_1", MarkerArray, queue_size=1)
	waypoint_pub = rospy.Publisher("/waypoint_1", Int32MultiArray, queue_size=1)
	dir_pub=rospy.Publisher("/link_direction_1", StringArray, queue_size=1)
	global_path_pub=rospy.Publisher("/optimal_frenet_path_global_1", PathArray, queue_size=1)
	col_pub=rospy.Publisher("/col_1", Int32, queue_size=1)
	...
	while not rospy.is_shutdown():
		...
		print("현재 링크 번호: "+ str(link_ind['global'])+", 링크 방향: "+str(find_dir(use_map.link_dir, link_ind['global']))+", target_speed: "+str(use_map.target_speed[mode][dir]))
