 ...
 * $Id: pcbtrace.c 13816 2022-01-18 11:26:41Z Fabrice $
 *
...
#define EPOCH_TRC_FILE		(time_t)-2209162161LL	//30/12/1899
#define DAY_IN_MILLISEC		86400000.0L
#define FRAC_PRECISION_DAY	10000000000

#define GET_DAY_IN_MILLISEC(s,ns)	((((s - EPOCH_TRC_FILE) % 86400) * 1000 + (ns / 1000000)))
...
int pcbtrace_write_header(struct pcbtrace_ctx *ctx, enum pcbtrace_version version) {
	...
	snprintf(buf, PCBTRACE_MAX_MSG, ";$STARTTIME=%" PRId64 ".%ld\n", 
		(int64_t)((ctx->time_start.tv_sec - EPOCH_TRC_FILE) / 86400), 
		FRACTION_OF_DAY(t->tm_hour * 3600 + t->tm_min * 60 + t->tm_sec, ctx->time_start.tv_nsec));
	n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
	if (n <= 0)
		return errno;
	/* columns */
