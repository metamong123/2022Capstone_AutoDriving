 ...
 * $Id: pcbcore.c 13816 2022-01-18 11:26:41Z Fabrice $
 *
...
TPCANStatus pcanbasic_read_common(
	...
	if (check_filter != 0) {
		int ignore = 0;
		__u32 opt_msgs = 0;
		int ires = pcanfd_get_option(pchan->fd, PCANFD_OPT_ALLOWED_MSGS, &opt_msgs, sizeof(opt_msgs));
		if (ires >= 0) {
			switch (msg.type) {
			case PCANFD_TYPE_CAN20_MSG:
	...
	case PCAN_ATTACHED_CHANNELS:
		...
		if (len < size) {
			...
			for (i = 0; i < g_basiccore.devices->length; ++i) {
				...
				pinfo = &g_basiccore.devices->infos[i];
				channel = pcanbasic_get_handle(pinfo->path, g_basiccore.devices);
				pchan = pcanbasic_get_channel(channel, 1, NULL);

				chi->channel_condition = (pinfo->bus_state == PCANFD_UNKNOWN) ? PCAN_CHANNEL_AVAILABLE : PCAN_CHANNEL_OCCUPIED;
				chi->channel_handle = channel;
...
#pragma GCC diagnostic pop
				...
				/* get a file descriptor even if channel is not initialized */
				fd = pcanbasic_get_fd(chi->channel_handle, pchan, &pinfo, &fd_close);
				if (fd > -1) {
					ires = pcanfd_get_option(fd, PCANFD_OPT_CHANNEL_FEATURES, &itmp, sizeof(itmp));
					if (ires >= 0) {
						if (itmp & PCANFD_FEATURE_FD)
