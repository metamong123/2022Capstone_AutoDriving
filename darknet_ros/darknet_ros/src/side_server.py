...
class YoloPub():
	...
	def delivery_vote(self, queue):
		if queue.count(0) > int(self.queue_size/2):
			return 0
		else:
	...
	def msg_pub(self):
		...
		ser.write(chr(47))
		ser.write(ser_data)
		for i in range(1, len(final_check.data)):
			final_data = final_check.data[i]
			data_100 = int(final_data / 100)
			data_10 = int(final_data / 10) % 10
			data_1 = int(final_data % 10)
			ser.write(chr(data_100))
			ser.write(chr(data_10))
			ser.write(chr(data_1))
			...
			if bounding_box.probability >= self.threshold:
				...
				if width < 300 and height < 300:
					...
					if bounding_box.id in (0, 1, 2): # find A number
						self.id_to_queue_list[0].append(bounding_box.id + 1)
